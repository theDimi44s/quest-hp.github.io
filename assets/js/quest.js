...
// === ЗАВАНТАЖЕННЯ (Оновлено для підтримки повернення з Квіддичу) ===
async function loadQuest(id){
  try {
    ...
    if (questData.questions) {
        Object.values(questData.questions).forEach(q => {
            if (q.image) { const img = new Image(); img.src = q.image; }
        });
    }

    // === ПЕРЕВІРКА: Чи ми повернулися з Квіддичу? ===
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('from') === 'quidditch') {
        // Відновлюємо дані з пам'яті телефону
        const savedScore = localStorage.getItem('hp_quest_score');
        const savedWinner = localStorage.getItem('hp_quest_winner');
        
        if (savedScore && savedWinner) {
            score = JSON.parse(savedScore);
            let winner = savedWinner;
            
            // Одразу переходимо до екрану перед Сферою (пропускаємо все попереднє)
...
// === 3. ПАЗЛ (Оновлено: перехід на Квіддич) ===
function startPuzzleTask(winner) {
    ...
    startPuzzle(questEl, () => { 
        ...
        // --- ЗБЕРІГАЄМО ДАНІ ТА ЙДЕМО НА КВІДДИЧ ---
        localStorage.setItem('hp_quest_score', JSON.stringify(score));
        localStorage.setItem('hp_quest_winner', winner);
...
function primeAudio() {
    houseAudio.src = silentMp3; houseAudio.volume = 1.0; 
    houseAudio.play().then(() => { houseAudio.pause(); }).catch(e => {});
}

function playHouseAudio(code){
    const src = audioFiles[code]; if(!src) return;
    houseAudio.src = src; houseAudio.currentTime = 0; houseAudio.volume = 1.0;
