...
const musicManager = {
    ...
    // ОНОВЛЕНО: Гарантована зупинка
    stopFadeOut(duration = 2000) {
        // 1. Пробуємо плавно
        this.fadeTo(0, duration);
        
        // 2. Але ставимо запобіжник: через вказаний час ТУПО ПАУЗА
        // Це вирішує проблему iOS, де fadeTo може застрягти
        setTimeout(() => {
            if (this.fadeInterval) clearInterval(this.fadeInterval);
            this.audio.pause();
        }, duration + 100);
...
function startOrbTask(winner) {
    ...
    startOrb(questEl, winner, 
        ...
        () => { 
            // 1. Починаємо глушити музику (гарантовано зупиниться через 2с)
            musicManager.stopFadeOut(2000); 
            // 2. Прогріваємо плеєр для голосу
            primeAudio();
        }
    );
}
...
function renderResult(winner){
  ...
  // Ще раз гарантовано зупиняємо фон (на всяк випадок)
  musicManager.stop();

  // Запускаємо голос факультету
...
// Тільки прогріває тишею, нічого не завантажує
function primeAudio() {
    houseAudio.src = silentMp3;
    houseAudio.volume = 1.0; 
    houseAudio.play().then(() => {
        // Одразу пауза. Браузер тепер думає, що ми "граємо".
        houseAudio.pause();
    }).catch(e => {});
}
